...
(`~/sportnis`). Наружу торчит только контейнер `web` на `127.0.0.1:8090`; публично его
...
4. Проверить: `curl -I http://127.0.0.1:8090` на сервере (контейнер web), затем `http://sportnis.ru`.
